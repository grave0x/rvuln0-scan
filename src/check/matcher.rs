...
mod tests {
    ...
    fn test_status_matcher_positive() {
        let c = Check {
            id: "t".into(), name: "test".into(), severity: Severity::Low, description: "".into(),
        ...
        assert!(matches(&c, &probe(200, HashMap::new(), "", None)));
    }
    ...
    fn test_header_present() {
        ...
        assert!(matches(&c, &probe(200, make_headers(vec![("server", "nginx")]), "", None)));
    }
    ...
    fn test_body_contains() {
        ...
        assert!(matches(&c, &probe(200, HashMap::new(), "admin panel", None)));
    }
    ...
    fn test_and_logic_all_match() {
        let c = Check {
            id: "t".into(), name: "test".into(), severity: Severity::Low, description: "".into(),
            matchers: Matchers {
                status: Some(vec![200]), header_present: Some(vec!["server".into()]),
                header_absent: Some(vec!["x-frame-options".into()]),
                body_regex: None, body_contains: Some(vec!["admin".into()]),
                title_contains: Some(vec!["Dashboard".into()]),
            },
        };
        let p = probe(200, make_headers(vec![("server", "nginx")]), "admin panel", Some("Admin Dashboard"));
        assert!(matches(&c, &p));
    }
